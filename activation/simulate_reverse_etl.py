...
from activation.destinations.hubspot_stub import send_to_hubspot
...
def fetch_segment(seg: str):
    ...
    if seg == "all":
        q = ("select customer_id, email, segment, score "
             "from main_marts.mart_marketing__segment_scores "
    ...
    con.close()
    return [
        {"customer_id": r[0], "email": r[1], "segment": r[2], "score": float(r[3])}
        for r in rows
    ]
